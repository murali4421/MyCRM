...
export class ActivityModalComponent {
    ...
    effect(() => {
      if (this.isAutoGenerated()) {
        ...
        untracked(() => {
          if (editingActivity) {
            this.isNew = false;
            // The `startTime` from data is a full ISO string, but datetime-local input needs `YYYY-MM-DDTHH:mm`.
            const localDateTime = new Date(editingActivity.startTime).toISOString().slice(0, 16);
            this.activityModel.set({ ...editingActivity, startTime: localDateTime });
          } else {
            this.isNew = true;
            this.activityModel.set({
              ownerId: this.authService.currentUser()?.id,
  ...
  async saveActivity(form: NgForm) {
    ...
    // Ensure startTime is a full ISO string and set endTime.
    const activityData = {
        ...formData,
