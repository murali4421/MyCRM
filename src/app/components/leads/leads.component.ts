  ...
  template: `
    <div>
      <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
          <h1 class="text-3xl font-bold text-gray-100">Leads</h1>
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button (click)="uiService.activeColumnCustomization.set('leads')" class="bg-gray-700 border border-gray-600 text-gray-200 px-4 py-2 rounded-md hover:bg-gray-600 text-sm font-medium">Columns</button>
            <button (click)="uiService.openLeadModal(null)" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 text-sm font-medium">
                Add Lead
            </button>
          </div>
      </div>
          ...
          <thead class="bg-gray-900">
            ...
            <tbody class="bg-gray-800 divide-y divide-gray-700">
              @for (lead of paginatedLeads(); track lead.id) {
                <tr class="hover:bg-gray-700 cursor-pointer" (click)="uiService.openLeadModal(lead)">
                  @if (isColumnVisible('name')) {
                    <td class="px-4 py-3 text-sm font-medium text-gray-100">{{lead.name}}</td>
                  }
                  @if (isColumnVisible('companyName')) {
                    <td class="px-4 py-3 text-sm text-gray-300">{{lead.companyName}}</td>
                  }
                  @if (isColumnVisible('email')) {
                    <td class="px-4 py-3 text-sm text-gray-300">{{lead.email}}</td>
                  }
                  @if (isColumnVisible('status')) {
                    <td class="px-4 py-3 text-sm text-gray-300"><span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" [class]="getStatusBadgeClass(lead.status)">{{ lead.status }}</span></td>
                  }
                  @if (isColumnVisible('source')) {
                    <td class="px-4 py-3 text-sm text-gray-300">{{lead.source}}</td>
                  }
                   @if (isColumnVisible('owner')) {
                    <td class="px-4 py-3 text-sm text-gray-300">{{ dataService.getUserById(lead.ownerId)?.name }}</td>
                  }
                  <td class="px-4 py-3 text-sm text-right">
                    @if (lead.status !== 'Qualified') {
                      <button (click)="$event.stopPropagation(); uiService.openLeadConversionModal(lead)" class="text-indigo-400 hover:text-indigo-300 font-medium">Convert</button>
                    }
                  </td>
                </tr>
              }
              @empty {
                <tr>
                  <td [attr.colspan]="visibleColumns().length + 1" class="text-center py-8 text-gray-400">No leads found.</td>
                </tr>
              }
            </tbody>
        </table>
...
export class LeadsComponent {
  ...
  getStatusBadgeClass(status: LeadStatus): string {
    const map: Record<LeadStatus, string> = {
      [LeadStatus.New]: 'bg-sky-500/10 text-sky-300',
      [LeadStatus.Contacted]: 'bg-amber-500/10 text-amber-300',
      [LeadStatus.Qualified]: 'bg-emerald-500/10 text-emerald-300',
      [LeadStatus.Disqualified]: 'bg-rose-500/10 text-rose-300',
    };
    return map[status];
  }
}
